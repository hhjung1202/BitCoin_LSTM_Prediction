...
class Learning:
    ...
    def build_arch(self):
        with tf.device('/gpu:0'):
            with tf.variable_scope('Conv_layer'):
                conv1 = tf.layers.conv2d(inputs=self.X, filters=64, kernel_size=[3, 3], strides=(1,1), padding='valid', name='conv1', reuse=tf.AUTO_REUSE)
                conv1 = tf.nn.relu(conv1, name='conv1_relu')
                assert conv1.get_shape() == [conv1.get_shape()[0], 7, 7, 64]

                conv2 = tf.layers.conv2d(inputs=conv1, filters=256, kernel_size=[3, 3], strides=(1,1), padding='valid', name='conv2', reuse=tf.AUTO_REUSE)
                conv2 = tf.nn.relu(conv2, name='conv2_relu')
                assert conv2.get_shape() == [conv2.get_shape()[0], 5, 5, 256]

                conv_a1 = tf.layers.conv2d(inputs=conv2, filters=256, kernel_size=[3, 3], strides=(1,1), padding='same', name='conv_a1', reuse=False)
                conv_a1= tf.nn.relu(conv_a1)
                assert conv_a1.get_shape() == [conv_a1.get_shape()[0], 5, 5, 256]

                conv_a2 = tf.layers.conv2d(inputs=conv_a1, filters=256, kernel_size=[3, 3], strides=(1,1), padding='same', name='conv_a2', reuse=False)
                conv_a2 = tf.nn.relu(conv_a2)
                assert conv_a2.get_shape() == [conv_a2.get_shape()[0], 5, 5, 256]

                # assert convt.get_shape() == [convt.get_shape()[0], 5, 5, 256]

                conv3 = tf.layers.conv2d(inputs=conv_a2, filters=16, kernel_size=[3, 3], strides=(1,1), padding='valid', name='conv3', reuse=tf.AUTO_REUSE)
                conv3 = tf.nn.relu(conv3, name='conv3_relu')  
                assert conv3.get_shape() == [conv3.get_shape()[0], 3, 3, 16]
                ...
                assert conv4.get_shape() == [conv4.get_shape()[0], 3, 3, 1]

                lstm_input = tf.reduce_sum(conv4, axis=3)
                assert lstm_input.get_shape() == [lstm_input.get_shape()[0], 3, 3]
            ...
            with tf.variable_scope('FullyConnected_layer'):
                fc1 = tf.contrib.layers.fully_connected(self.lstm_output[:, self.output_dim], self.fn_output1, activation_fn=None)
    ...
    def run(self):
        ...
        # saver = tf.train.import_meta_graph("./saved_v6-2/test_model-3000.meta")
        ...
        #             if (epoch + 1) % 1000 == 0 :
        #                 self.savor.save(sess, './saved_v10/test_model', global_step=epoch+1)
